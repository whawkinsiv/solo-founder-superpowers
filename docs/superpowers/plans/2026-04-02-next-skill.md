# "next" Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a skill that scans a founder's project holistically and surfaces 3-5 high-value, low-effort opportunities ranked by impact, with routing to the right skill for each.

**Architecture:** Single SKILL.md file following the plugin's existing skill conventions. Signal-based approach — the skill guides Claude to read context files and scan the codebase, then use stage-aware judgment to find gaps across product, infrastructure, marketing, growth, and business. No supporting files needed for v1.

**Tech Stack:** Markdown (SKILL.md with YAML frontmatter)

**Spec:** `docs/superpowers/specs/2026-04-02-next-skill-design.md`

---

### Task 1: Create the "next" skill directory and SKILL.md
...
Create `skills/next/SKILL.md` with the following complete content:
...
```bash
cat skills/next/SKILL.md | head -5
```

Expected: YAML frontmatter with `name: next` and `description` field.

- [ ] **Step 4: Commit**

```bash
git add skills/next/SKILL.md
git commit -m "feat: add 'next' skill for discovering high-value opportunities"
```

---

### Task 2: Update plugin.json skill count

**Files:**
- Modify: `.claude-plugin/plugin.json`

- [ ] **Step 1: Update the description to reflect the new skill count**

In `.claude-plugin/plugin.json`, change the description from "54 expert skills" to "55 expert skills":

```json
"description": "55 expert skills for non-technical founders building SaaS with AI tools. Covers the full lifecycle: planning, design, build, database, integrations, AI features, security, testing, debugging, deployment, monitoring, plus marketing, pricing, growth, and business strategy."
```
...
git commit -m "chore: bump plugin description to 55 skills"
```
...
- [ ] **Step 1: Update the repository purpose line**

In `CLAUDE.md`, change "48 skills" to the current accurate count (verify by running `ls -d skills/*/SKILL.md | wc -l` first).

- [ ] **Step 2: Commit**

```bash
git add CLAUDE.md
git commit -m "chore: update CLAUDE.md skill count"
```
